...
@patch(
    "src.services.registration_service.generate_activation_code",
    return_value=TEST_ACTIVATION_CODE,
)
@patch(
...
def test_activate_user_success(
    mock_datetime, registration_service, user_repo_mock, token_repo_mock
):
    user = User(
        id=TEST_USER_ID,
        email=TEST_EMAIL,
    ...
    token = ActivationToken(
        user_id=TEST_USER_ID, code=TEST_ACTIVATION_CODE, created_at=TOKEN_CREATION_TIME
    )

    user_repo_mock.find_by_email.return_value = user
    token_repo_mock.find_by_user_id_and_code.return_value = token

    mock_datetime.now.return_value = TOKEN_CREATION_TIME + timedelta(
        seconds=ACTIVATION_TOKEN_TTL_SECONDS - 10
    )

    activated_user = registration_service.activate_user(
        TEST_EMAIL, TEST_ACTIVATION_CODE
    )

    assert activated_user.is_active()
    user_repo_mock.update_user_status.assert_called_once_with(
        TEST_USER_ID, UserStatus.ACTIVE.value
    )
...
def test_activate_user_invalid_token(
    registration_service, user_repo_mock, token_repo_mock
):
    user = User(
        id=TEST_USER_ID,
        email=TEST_EMAIL,
    ...
    user_repo_mock.find_by_email.assert_called_once_with(TEST_EMAIL)
    token_repo_mock.find_by_user_id_and_code.assert_called_once_with(
        TEST_USER_ID, "WRONGCODE"
    )


def test_activate_user_token_expired(
    registration_service, user_repo_mock, token_repo_mock
):
    user = User(
        id=TEST_USER_ID,
    ...
    with patch("src.services.registration_service.datetime") as mock_datetime:
        future_time = TOKEN_CREATION_TIME + timedelta(
            seconds=ACTIVATION_TOKEN_TTL_SECONDS + 10
        )
