...
def test_register_and_activate_user_success(client: TestClient):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT code FROM activation_tokens WHERE user_id = %s;", (user_id,))
...
def test_activate_user_token_expired(client: TestClient):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT code FROM activation_tokens WHERE user_id = %s;", (str(user.id),))
            activation_code = cur.fetchone()[0]
            
            expired_time = datetime.now(timezone.utc) - timedelta(seconds=ACTIVATION_TOKEN_TTL_SECONDS + 1)
            cur.execute(
